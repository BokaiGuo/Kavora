...
function renderDecision(decision) {
  ...
  const experiment = decision.experiment_id ? `${decision.experiment_id} / ${decision.assigned_policy} / ${decision.experiment_window}` : 'none';
  $('decision-inspector').innerHTML = `<div class="decision-summary"><span>MODE <strong>${decision.mode}</strong></span><span>EXPERIMENT <strong>${experiment}</strong></span><span>PREDICTOR <strong>${predictor}</strong></span><span>SELECTED <strong>${decision.selected || 'static fallback'}</strong></span><span>ACTUAL <strong>${decision.actual_selected || 'pending'}</strong></span><span>TTFT ERROR <strong>${error}</strong></span><span>REASON <strong>${decision.reason}</strong></span><span>REQUIREMENTS <strong>${requirements}</strong></span></div><div class="decision-table-wrap"><table><thead><tr><th>Backend</th><th>Eligible</th><th>Evidence</th><th>Confidence</th><th>Matched</th><th>Queue</th><th>TTFT ms</th><th>Score</th><th>Explanation</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
